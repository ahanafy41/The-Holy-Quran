 ...
 */
interface HadithAIAssistantModalProps {
    /** The Hadith object that is the context for the chat session. */
    hadith: Hadith;
...
/**
 * `HadithAIAssistantModal` provides an AI-powered chat interface for users to ask questions
 * and get explanations about a specific hadith. It uses the Google Gemini API for chat functionality
...
export const HadithAIAssistantModal: React.FC<HadithAIAssistantModalProps> = ({ hadith, onClose }) => {
    ...
    const suggestionPrompts = ["اشرح هذا الحديث بعبارات بسيطة", "ما هو سياق هذا الحديث؟", "ما هي الدروس الرئيسية المستفادة؟"];
                ...
                <motion.div ref={modalContentRef} {...modalAnimationProps}
                  onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-800 rounded-none sm:rounded-2xl shadow-xl w-full h-full sm:h-auto sm:max-w-2xl sm:max-h-[90vh] flex flex-col" role="dialog" aria-modal="true" aria-labelledby="ai-assistant-title">
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/50 flex-shrink-0">
                        <p className="text-sm text-slate-600 dark:text-slate-400">حول الحديث رقم {hadith.idInBook}:</p>
                        <p className="font-serif text-lg mt-1 text-right">{hadith.arabic}</p>
                    </div>
                    ...
                    <div className="flex-1 p-4 overflow-y-auto space-y-4" aria-live="polite">
                        ...
                        {messages.length === 0 && !isResponding && (
                            <motion.div {...suggestionsAnimationProps} className="text-center text-slate-500 dark:text-slate-400 py-8">
                                <p className="mb-4">كيف يمكنني مساعدتك في فهم هذا الحديث؟</p>
