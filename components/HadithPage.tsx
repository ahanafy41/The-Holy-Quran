...
export const HadithPage: React.FC = () => {
    ...
    const handleSelectBook = async (book: HadithBook) => {
        ...
        setError(null);
        try {
            const response = await fetch(`https://raw.githubusercontent.com/ahanafy41/The-Holy-Quran/main/hadith-data/${book.id}/chapters.json`);
    ...
    const handleSelectChapter = async (chapter: HadithChapter) => {
        ...
        setError(null);
        try {
            const response = await fetch(`https://raw.githubusercontent.com/ahanafy41/The-Holy-Quran/main/hadith-data/${selectedBook.id}/${chapter.id}.json`);
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            setHadiths(data);
            setView('hadiths');
