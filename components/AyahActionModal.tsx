...
import { WordMeaningsView } from './WordMeaningsView';
...
export const AyahActionModal: React.FC<AyahActionModalProps> = ({ ayah, onClose }) => {
    ...
    const [isShowingMeanings, setIsShowingMeanings] = useState(false);
    ...
    const menuItems = [
        ...
        { label: 'معاني الكلمات', icon: BookOpenIcon, action: () => setIsShowingMeanings(true) },
                    ...
                    >
                        <header className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center flex-shrink-0">
                            <h2 id="ayah-action-title" className="font-bold text-lg">
                                {isShowingMeanings ? `معاني كلمات الآية ${ayah.numberInSurah}` : `سورة ${ayah.surah?.name} - الآية ${ayah.numberInSurah}`}
                            </h2>
